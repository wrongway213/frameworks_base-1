...
package com.android.systemui.ambientmusic.soundrecognition;
 ...
 */
public class AmbientSongRecognition {

    private static final String TAG = "AmbientSongRecognition";
    ...
    private class RecorderThread extends Thread {
        ...
        private void parseXmlResult(String xml) {
            if (xml.contains("did not hear any music") || xml.contains("no close matches")) {
                // No result
                Log.d(TAG, "No match (Maybe we could not hear the track?)");
                reportResult(null);
            } else {
        ...
        private void reportResult(Results result) {
    ...
    /**
     * Creates an AmbientSongRecognition matching instances and allocates audio buffers
     */
    public AmbientSongRecognition(@NonNull Callback callback) {
        mCallback = callback;
